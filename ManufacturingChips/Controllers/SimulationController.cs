...
namespace ManufacturingChips.Controllers
{
    public class SimulationController : Controller
    {
        ...
        public IActionResult StartSimulation([FromBody] SimulationView vm)
        {
            ...
            {
                var rnd = new Random();
                var endTime = DateTime.UtcNow + TimeSpan.FromSeconds(vm.ShiftDurationSeconds);
            ...
            _lineTasks = lines
                .Select((ln, idx) =>
                    Task.Run(() => ln.Start(_inputQueue, TimeSpan.Zero, token), token)
                )
                .ToArray();
            ...
            Task.Run(async () =>
            {
                try
                {
                    ...
                    _latestStats = lines
                        .Select((ln, idx) => ln.CollectStatistics(idx + 1,
                            TimeSpan.FromSeconds(vm.ShiftDurationSeconds)))
                        .ToList();
                }
                catch (OperationCanceledException) { }
                finally
